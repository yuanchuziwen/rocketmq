...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    // 初始化 mqFaultStrategy，即 broker 故障延迟机制
    // 每次发送失败，都会将该 brokerName 加入到 latencyFaultTolerance 中
    ...
    public void start(final boolean startFactory) throws MQClientException {
        ...
        // 注册定时任务，定时扫描 requestFutureTable，移除超时的请求
    ...
    private SendResult sendDefaultImpl(
        ...
        // 验证此时 producer 的 serviceState 正常
        this.makeSureStateOK();
        // 检查消息的合法性
        Validators.checkMessage(msg, this.defaultMQProducer);
        // 获取一个随机值来标记本次发送，只用打日志
        ...
        if (topicPublishInfo != null && topicPublishInfo.ok()) {
            ...
            for (; times < timesTotal; times++) {
                ...
                // 选择一个 MessageQueue（挑选的逻辑会结合 broker 的负载情况）
                MessageQueue mqSelected = this.selectOneMessageQueue(topicPublishInfo, lastBrokerName);
                if (mqSelected != null) {
                    // 记录本次发送的 MessageQueue
                    mq = mqSelected;
                    brokersSent[times] = mq.getBrokerName();
                    try {
                        // 更新 msg 上的时间信息、topic 等
                        ...
                        switch (communicationMode) {
                            ...
                            case ONEWAY:
                                return null;
                            case SYNC:
                                // 如果成功请求了，但是响应中的状态不是 SEND_OK，那么根据配置决定是否重试
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        // 从 topicPublishInfoTable 中获取 topic 对应的 TopicPublishInfo（本地获取）
        TopicPublishInfo topicPublishInfo = this.topicPublishInfoTable.get(topic);
        if (null == topicPublishInfo || !topicPublishInfo.ok()) {
            this.topicPublishInfoTable.putIfAbsent(topic, new TopicPublishInfo());
            // 从 name server 更新 topic 的路由信息
            this.mQClientFactory.updateTopicRouteInfoFromNameServer(topic);
            topicPublishInfo = this.topicPublishInfoTable.get(topic);
        }

        // 如果 topicPublishInfo 有路由信息，或者 topicPublishInfo 是 ok 的，那么直接返回
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            return topicPublishInfo;

            // 否则，也需要从 name server 更新 topic 的路由信息
        } else {
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        // 在 MQClientInstance 中根据 messageQueue 获取 brokerName
        String brokerName = this.mQClientFactory.getBrokerNameFromMessageQueue(mq);
        // 在 MQClientInstance 中根据 brokerName 获取 brokerAddr
        String brokerAddr = this.mQClientFactory.findBrokerAddressInPublish(brokerName);

        // 如果无法获取 brokerAddr，那么尝试从 name server 更新 topic 的路由信息
        if (null == brokerAddr) {
        ...
        if (brokerAddr != null) {
            // 基于是否使用 vip 通道，修正 brokerAddr
            brokerAddr = MixAll.brokerVIPChannel(this.defaultMQProducer.isSendMessageWithVIPChannel(), brokerAddr);

            byte[] prevBody = msg.getBody();
            try {
                //for MessageBatch,ID has been set in the generating process
                // 如果不是批量消息，那么为消息设置唯一 ID
                if (!(msg instanceof MessageBatch)) {
                    MessageClientIDSetter.setUniqID(msg);
                }

                // 为 ms 设置 client 的 instanceId
                ...
                // 如果消息是事务消息，那么设置事务消息标志
                final String tranMsg = msg.getProperty(MessageConst.PROPERTY_TRANSACTION_PREPARED);
                if (Boolean.parseBoolean(tranMsg)) {
                    sysFlag |= MessageSysFlag.TRANSACTION_PREPARED_TYPE;
                }

                // 如果有 checkForbiddenHook，那么执行 checkForbiddenHook
                ...
                // 如果有 sendMessageHook，那么执行 sendMessageHook
                ...
                // 如果 topic 是以 retry 开头的；则需要计算 reconsumeTimes 和 maxReconsumeTimes
        ...
        // 如果从 name server 更新 topic 之后，还是无法获取 brokerAddr，那么抛出异常
    ...
    public SendResult send(Message msg, MessageQueue mq, long timeout)
        ...
        // 这里直接调用了 sendKernelImpl 方法，而不是 sendDefaultImpl 方法；也就是说这个方法不支持 retry
    ...
    /*
    Send 方法可以划分为几个层级：
    1. public 的 send 方法；会调用 2 或者 3
    2. sendDefaultImpl、sendSelectorImpl；都会调用 3
    3. sendKernelImpl（真正的 send 实现）
     */
